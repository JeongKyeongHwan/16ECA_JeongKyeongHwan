...
def fwd_euler(f, x_init, t_start, t_end, delta_t):
    ...
    for k in list_k[:-1]:
        ...
        for i in xrange(n_states):
            # 전진 오일러법을 적용
            xk[i] = xk[i] + sk [i] *delta_t

        # xk값을 다음 상태 값으로 갱식
        xk = xk1

        return list_t, list_x
    def mod_euler(f, x_inti, t_start, t_end, delta_t):
        """
...
def runge_while(f, x_init, t_init, t_end, delta_t):
    ...
    # x_list[0]은 상태 변수의 초기값으로 바뀌지 않을 것이므로 tuple형르ㅗ 저장
    x_list = [x_init]
    t_list = [t_init]
...
def runge_while(f, x_init, t_init, t_end, delta_t):
    ...
    Examples
    --------
    >>> list_t, list_x = runge_while(lambda x, t:[math.sin(math.pi*t) - [0])*0.5],[0],0.0, 0.5, 0.1)
    >>> print list_t
...
def main():
    ...
    # exact soulution
    vXexact = tuple([exact(kt) for tk in vT])
